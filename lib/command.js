...
Expecting one of '${allowedValues.join("', '")}'`);
  ...
  */
  ...
  complete(rules) {
    ...
    // support both arguments or arguments as key
    if (rules.arguments) {
  ...
  hasCompletionRules() {
    function isEmptyRule(rules) {
      return Object.keys(rules.options).length === 0 &&
        Object.keys(rules.args).length === 0;
    }

    return !(
      isEmptyRule(this._completionRules) &&
      this.commands.every((command) => isEmptyRule(command._completionRules))
    );
  }
  ...
  autocompleteHandleEvent(event) {
    if (this.commands.length > 0) {
      ...
      if (event.fragment === 1) {
        ...
        const currentCommand = this.commands.find((c) => c.name() === commandName);
  ...
  autocompleteCandidates(typedArgs) {
    ...
    const optionNames = Object.keys(options).filter((name) => {
    ...
    const activeArg = this._autocompleteActiveArg(options, args, typedArgs);
 ...
 */
  ...
  _autocompleteActiveOption(optionRules, typedArgs) {
 ...
 */
  ...
  _autocompleteActiveArg(optionRules, argRules, typedArgs) {
